...
class CandidateTokenRequired(HTTPBearer):
    ...
    def __init__(self, token: Optional[str] = Query(None), roles=None, auto_error: bool = True):
        ...
        self.roles = roles
        self.token = token
        super(CandidateTokenRequired, self).__init__(auto_error=auto_error)

    async def __call__(self, request: Request, db: Session = Depends(get_db)):
        required_roles = self.roles
        credentials: HTTPAuthorizationCredentials = await super(CandidateTokenRequired, self).__call__(request)
        ...
        if credentials:
            ...
            current_user = crud.candidate.get_by_uuid(db=db, uuid=token_data["sub"])
            if not current_user:
                raise HTTPException(status_code=403, detail=__("dependencies-token-invalid"))
