...
class CRUDApplication(CRUDBase[models.Application, schemas.ApplicationDetails, schemas.ApplicationResponse]):
    ...
    def get_by_uuid(cls, db: Session, *, uuid: str):
        return db.query(models.Application).filter(models.Application.uuid == uuid,
                                                   models.Application.is_deleted == False).first()

    @classmethod
    def get_by_candidate_uuid(cls, db: Session, *, candidate_uuid: str):
        return db.query(models.Application).filter(models.Application.candidate_uuid == candidate_uuid,
                                                   models.Application.is_deleted == False).all()

    @classmethod
    def get_by_job_offer_uuid(cls, db: Session, *, job_offer_uuid: str):
        return db.query(models.Application).filter(models.Application.job_offer_uuid == job_offer_uuid,
                                                   models.Application.is_deleted == False).all()
